...
/* A wallet whose key lives in the app, used only when VITE_TEST_WALLET is set,
   so a browser test can sign without driving a real extension. The seed is
   fixed so the same test wallet comes back every run. */
...
/* Signs the challenge and lands a session cookie. A wallet the platform has
   authorised as a custodian comes back with the staff role and reaches the
   console; any other wallet is a member and is turned away by the staff gate. */
export function useWalletSignIn(options: { onSuccess: () => Promise<void> | void }): {
  ...
  const mutation = useMutation<void, unknown, void>({
    mutationFn: async () => {
      ...
      const challenge = await beginWalletSignIn({ address });
      /* Sign on whatever chain the wallet already holds this account on: proving
         control of the key is the same on every Sui network. Left unset, dapp-kit
         forces `sui:${defaultNetwork}` and a wallet on another network rejects it. */
