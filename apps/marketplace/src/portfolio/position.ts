    ...
    return {
      ...
      caption:
        listing.offerCount === 0
          ...
          : `${listing.offerCount} lender${listing.offerCount === 1 ? '' : 's'} competing`,
      figure:
...
export function positionOfBorrowedLoan(loan: LoanResponse, now: number): Position {
  ...
  if (loan.status === 'ACTIVE') {
    ...
    return {
      ...
      caption: `You borrowed at ${rateOf(loan.annualPercentageRateBasisPoints)} p.a.`,
...
export function positionOfLentLoan(loan: LoanResponse, now: number): Position {
  ...
  if (loan.status === 'ACTIVE') {
    ...
    return {
      ...
      stage: 'Earning',
      tone: 'active',
      caption: `You lent at ${rateOf(loan.annualPercentageRateBasisPoints)} p.a.`,
